...
export const VideoFeed = (props: Props) => {
  ...
  useEffect(() => {
    const keyboardConnectionStatus = new protos.Input({
      connected: isKeyboardModeToggled,
      axes: [],
      buttons: 0,
      source: protos.Source.KEYBOARD
    })
    webSocket.send(protos.UserInputs.encode({ inputs: keyboardConnectionStatus }).finish());
//     ipcRenderer.send('stateUpdate', [keyboardConnectionStatus], Source.KEYBOARD);
  }, [isKeyboardModeToggled, webSocket]);

  const sendKeyboardInputsToRuntime = (keyboardBitmap_: number) => {
    const keyboard = new protos.Input({
      connected: true,
      axes: [],
      buttons: keyboardBitmap_,
      source: protos.Source.KEYBOARD
    });
    webSocket.send(protos.UserInputs.encode({ inputs: keyboard }).finish());
  };
